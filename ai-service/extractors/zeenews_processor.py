...
def get_previously_processed_urls():
    ...
    articles_json_file = os.path.join("output", "zeenews_articles_processed.json")
    ...
    if os.path.exists(links_json_file):
        try:
            with open(links_json_file, 'r', encoding='utf-8') as f:
                ...
                # Get only Zee News links
                links = all_links.get('zeenews', [])
        except Exception as e:
...
def clean_content(content):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            # Skip common navigation/footer elements specific to Zee News
            if any(nav in line.lower() for nav in [
                'zee news', 'follow us on', 'top stories', 'most popular',
                'trending news', 'subscribe to', 'read more news', 'next story',
                'advertisement', 'related stories', 'share this article', 'comments',
                'facebook', 'twitter', 'linkedin', 'whatsapp', 'reddit', 'zee media',
                'sign in', 'subscribe now', 'download app', 'watch live',
                'copyright', 'privacy policy', 'terms of use', 'all rights reserved',
                'trending now', 'popular categories', 'newsletter', 'daily news',
                'zeebiz', 'dna india', 'wion', 'zee business', 'zee hindustan',
                'disclaimer', 'partner sites', 'contact us', 'advertise with us'
            ]):
                continue
            ...
            location_pattern = r'^[A-Z]{3,}[,:]|^New Delhi[,:]|^Mumbai[,:]|^Kolkata[,:]'
            ...
            if re.match(r'^By\s+|^Zee Media Bureau|^Written By|^Edited By|^Reported By', line):
                continue
        ...
        content = re.sub(r'Photo:\s*[^.]+\.', '', content)  # Remove photo credits
        content = re.sub(r'Image:\s*[^.]+\.', '', content)  # Remove image credits
...
def find_best_image_url(soup):
    ...
    try:
        ...
        # Check for article main image - Zee News specific selectors
        if not main_image:
            article_image = soup.select_one('.article-image img, .articleImage img, .article_image img, .main-image img')
        ...
        if not main_image:
            for img in soup.find_all('img'):
                ...
                if any(skip in (img.get('src', '') or '').lower() for skip in ['logo', 'icon', 'advertisement']):
                    continue
...
def extract_content_from_url(url):
    ...
    try:
        ...
        headline = ""
        headline_selectors = [
            'h1.article-heading',  # Primary headline selector
            'h1.articleheading',   # Alternative headline selector
            'h1.story-heading',    # Alternative headline selector
        ...
        for selector in headline_selectors:
            ...
            if headline_elem:
                ...
                headline = headline.replace(' | Zee News', '')  # Remove site name
                headline = headline.replace(' - Zee News', '')
                headline = headline.replace(' | Latest News', '')
                if headline:
                    break
        ...
        if not headline:
            ...
            if meta_title:
                ...
                headline = re.sub(r'\s*[|]\s*Zee News.*$', '', headline)
            else:
        ...
        content = ""
        content_selectors = [
            'div.article-body',         # Primary content selector
            'div.articleBody',          # Alternative content selector
            'div.story-details',        # Alternative content selector
            'div[itemprop="articleBody"]',  # Schema.org content
            'div.article_content',      # Legacy content selector
            'div.article-content'       # Legacy content selector
        ]
        ...
        if date_meta:
            ...
            date_elem = soup.select_one('.article-date, .date, time[datetime]')
            if date_elem:
        ...
        # Extract the time
        time_str = None
        time_elem = soup.select_one('.article-date, .date, time[datetime]')
        if time_elem:
        ...
        for selector in author_selectors:
            ...
            if author_elem:
                ...
                author = re.sub(r'^By\s+|^Written by\s+|^Reported by\s+|^Edited by\s+|^Zee Media Bureau\s+', '', author)
                break
        ...
        breadcrumb = soup.select('.breadcrumb a, .breadcrum a, .category a')
        if breadcrumb:
...
def process_latest_article():
    ...
    try:
        ...
        output_file = os.path.join(base_dir, "zeenews_articles_processed.json")
        existing_articles = []
