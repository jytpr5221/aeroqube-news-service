...
def get_previously_processed_urls():
    ...
    articles_json_file = os.path.join("output", "aajtak_articles_processed.json")
    ...
    if os.path.exists(links_json_file):
        try:
            with open(links_json_file, 'r', encoding='utf-8') as f:
                ...
                # Get only Aaj Tak links
                links = all_links.get('aajtak', [])
        except Exception as e:
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if any(nav in line.lower() for nav in [
                ...
                'breaking news', 'top stories', 'live tv', 'photo gallery',
                'download app', 'aaj tak', 'india today', 'अन्य भाषाओं में पढ़ें',
                'अधिक पढ़ें', 'देखें वीडियो', 'शेयर करें', 'ताज़ा खबरें'
            ]):
                continue
            ...
            if re.match(r'^By\s+|^द्वारा\s+', line) or "आजतक" in line:
                continue
            
            # Skip common Hindi markers
            if "अपडेटेड" in line or "प्रकाशित" in line:
                continue
...
def find_best_image_url(soup):
    ...
    try:
        ...
        # Check for article main image - AajTak specific selectors
        if not main_image:
            # AajTak specific selectors
            article_image = soup.select_one('.artical-big-img img, .story-heading-image img, .story-image img, .imgBorder img')
...
def find_video_url(soup, url):
    ...
    try:
        ...
        url_lower = url.lower()
        is_video_page = any(v in url_lower for v in ['/video/', 'watch-video', 'video-', 'tv/video'])
...
def extract_content_from_url(url):
    ...
    try:
        ...
        headline = ""
        headline_selectors = [
            'h1.artical-title', 'h1.story-heading', 'h1.title', 'h1',
            '.article-title', '.title', '.story_heading',
        ...
        # APPROACH 1: Try all possible content selectors
        content_selectors = [
            # Original selectors
            '.story_details p', '.story-details p', '.desc p',
            '.artical-content p', '.article-content p',
            '.video-content p', '.video_content p',
            '.story-desc p',
            # Additional selectors found in analysis
            'p.description', '.article__content p', '.content p',
            '.story-content p', '.news-detail p',
            'div[itemprop="articleBody"] p',
            '.storydetailpara p',
            # Generic selector for all paragraphs in main sections
            'main p', 'article p', '.main-content p', '#content p'
        ]
        ...
        # APPROACH 2: If still no content, try meta description + first few paragraphs
        ...
        # APPROACH 3: If still no good content, try looking for text directly in the body
        if not content or len(content) < 100:
            ...
            if main_divs:
                ...
                content = clean_content(main_div_text)
        ...
        if author_meta and author_meta.get('content'):
            ...
            for selector in ['.author', '.byline', '.reporter-name', 'span.author-name']:
        ...
        if len(url_parts) > 3:
            ...
            if possible_category and possible_category not in ['video', 'index.html']:
        ...
        # If no image found, try Open Graph image
        if not image_url:
            og_image = soup.select_one('meta[property="og:image"]')
            if og_image and og_image.get('content'):
        ...
        # Determine language (Hindi or English)
        language = 'hi'  # Default to Hindi for AajTak
        
        # Check if content contains significant Hindi text
        hindi_chars = sum(1 for c in content if ord(c) >= 0x0900 and ord(c) <= 0x097F)
        if hindi_chars < len(content) * 0.1:  # Less than 10% Hindi characters
            language = 'en'  # Probably an English article
...
def process_latest_article():
    ...
    if article:
        ...
        output_file = os.path.join(base_dir, "aajtak_articles_processed.json")
        existing_articles = []
