...
def get_previously_processed_urls():
    ...
    articles_json_file = os.path.join("output", "ndtv_articles_processed.json")
...
def clean_content(content, lang='en'):
    ...
    try:
        ...
        common_nav_terms_en = [
            ...
            'download app', 'ndtv', 'ndtv india', 'ndtv profit'
        ]
        ...
        common_nav_terms_hi = [
            ...
            'एनडीटीवी', 'एनडीटीवी इंडिया', 'डाउनलोड'
        ]
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r'^By\s+|^द्वारा\s+', line) or "NDTV" in line or "ndtv" in line.lower():
                continue
...
def find_best_image_url(soup, url):
    ...
    try:
        ...
        # Check for article main image - NDTV specific selectors
        if not main_image:
            article_image = soup.select_one('.article_image img, .ins_instory_dv_cont img, .story_img img, .featured_image img, .big-thumb img, .article-img img')
        ...
        if not main_image:
            for img in soup.find_all('img'):
                ...
                # If no dimensions but has src and class has 'main', 'featured', 'article', etc.
                    ...
                    if any(term in class_str.lower() for term in ['main', 'featured', 'article', 'story', 'img-responsive']):
                        main_image = img.get('src')
                        break
        ...
        if main_image and not main_image.startswith(('http://', 'https://')):
            ...
            base_url = "https://www.ndtv.com" if "ndtv.com" in url else "https://khabar.ndtv.com"
...
def find_video_url(soup, url):
    ...
    try:
        ...
        if is_video_page:
            ...
            video_elem = soup.select_one('video source, iframe[src*="youtube"], iframe[src*="ndtv"], iframe[src*="dailymotion"]')
            if video_elem:
...
def detect_language(url, content):
    ...
    if "khabar.ndtv" in url or "hindi.ndtv" in url:
        return 'hi'  # Hindi
...
def extract_content_from_url(url):
    ...
    try:
        ...
        headline = ""
        headline_selectors = [
            'h1.entry-title', 'h1.sp-title', 'h1.headline', 'h1',
            '.entry-title', '.sp-title', '.headline', '.story_title',
        ...
        if not headline:
            ...
            if soup.title:
                ...
                headline = re.sub(r'\s*\|\s*NDTV.*$', '', headline)
                headline = re.sub(r'\s*\|\s*एनडीटीवी.*$', '', headline)
        ...
        # Try NDTV-specific content selectors first
        content_selectors = [
            # NDTV specific selectors
            '.story__content p', '.story_details p', '.content_text p',
            '.sp-cn p', '.story-content p', '.ins_storybody p', 
            '#content_id p', '.article-txt p', '.story-details p',
        ...
        author = "NDTV" if language == 'en' else "NDTV Hindi"  # Default author
        ...
        if author_meta and author_meta.get('content'):
            ...
            for selector in ['.article__byline', '.author', '.byline', '.reporter-name', '.posted-by']:
        ...
        # Extract the source
        source = "NDTV" if language == 'en' else "NDTV Hindi"
        ...
        url_parts = url.split('/')
        if len(url_parts) > 3:
            possible_category = url_parts[3]  # Usually the category is after domain
...
def process_latest_article():
    ...
    if article:
        ...
        output_file = os.path.join(base_dir, "ndtv_articles_processed.json")
        existing_articles = []
